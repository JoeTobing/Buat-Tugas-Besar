...
-- File SQL untuk import ke phpMyAdmin
-- Import file ini setelah membuat database 'servis_elektronik'
...
CREATE TABLE `notifications` (
  ...
  `body` text NOT NULL,
  `type` enum('ORDER_UPDATE','MESSAGE','PAYMENT','SYSTEM') NOT NULL DEFAULT 'SYSTEM',
...
CREATE TABLE `orders` (
  ...
  `category` enum('COMPUTER','SMARTPHONE','HOME_APPLIANCE','OTHER') NOT NULL,
  `device_model` varchar(255) DEFAULT NULL,
  `problem_description` text NOT NULL,
  `service_type` enum('AT_SHOP','HOME_SERVICE') NOT NULL,
  `address` text NOT NULL,
  `latitude` decimal(10,8) DEFAULT NULL,
  `longitude` decimal(11,8) DEFAULT NULL,
  `status` enum('PENDING','IN_NEGOTIATION','ACCEPTED','ON_PROGRESS','COMPLETED','REJECTED','CANCELLED') NOT NULL DEFAULT 'PENDING',
...
CREATE TABLE `payments` (
  ...
  `method` enum('CASH','TRANSFER','EWALLET','OTHER') NOT NULL DEFAULT 'CASH',
...
CREATE TABLE `technicians` (
  ...
  `latitude` decimal(10,8) DEFAULT NULL,
  `longitude` decimal(11,8) DEFAULT NULL,
  `category` enum('COMPUTER','SMARTPHONE','HOME_APPLIANCE','OTHER') NOT NULL DEFAULT 'OTHER',
  `description` text DEFAULT NULL,
  `base_price` decimal(10,2) DEFAULT NULL,
  `status` enum('AVAILABLE','ON_DUTY','OFFLINE') NOT NULL DEFAULT 'OFFLINE',
...
ALTER TABLE `order_photos`
  ADD PRIMARY KEY (`id`),
  ADD KEY `order_photos_order_id_foreign` (`order_id`);
...
--
ALTER TABLE `order_photos`
  ADD CONSTRAINT `order_photos_order_id_foreign` FOREIGN KEY (`order_id`) REFERENCES `orders` (`id`) ON DELETE CASCADE;
...
ALTER TABLE `orders`
  ...
  ADD CONSTRAINT `orders_technician_id_foreign` FOREIGN KEY (`technician_id`) REFERENCES `technicians` (`id`) ON DELETE SET NULL;
...
--
ALTER TABLE `services`
  ADD CONSTRAINT `services_technician_id_foreign` FOREIGN KEY (`technician_id`) REFERENCES `technicians` (`id`) ON DELETE CASCADE;
